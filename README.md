...
4. 全程有進度：偵測到 SD 卡就在右下角開一個小視窗（階段、目前檔名、進度條、已複製 GB 與剩餘時間）；結束時視窗顯示結果，成功 15 秒後自動關閉，失敗留著（最多 10 分鐘）。關掉視窗不影響備份。
5. Windows 通知中心的通知預設關閉（`$ToastMode = 'None'`）。改 `ErrorsOnly` 只在失敗時發一張停在畫面上的通知；改 `All` 則偵測到、複製中（同一張進度條原地更新）、結果都發。
...
| `$RetentionApply` | `$true` | **清理總開關**。第一次先 `$false` 預覽，看過 log 確認後才改 `$true`（2026-09-04 已確認） |
| `$ToastMode` | `None` | 通知中心通知：`None`／`ErrorsOnly`／`All` |
